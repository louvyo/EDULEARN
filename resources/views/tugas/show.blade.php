        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-minimal overflow-hidden mb-6">
            ...
            <div class="p-8">
                <div class="flex items-start gap-4 mb-6">
                    <div
                        class="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <a href="{{ route('kelas.detail', $tugas->kelas_id) }}"
                                class="inline-flex items-center px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 text-sm font-semibold hover:bg-blue-100 transition-colors duration-200">
                    ...
                    @if ($isGuru ?? false)
                        <div class="flex items-center gap-2">
                            <a href="{{ route('tugas.edit', $tugas->id) }}"
                                class="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 hover:bg-blue-100 rounded-xl text-blue-700 font-semibold transition-all duration-300 hover:scale-105">
                            ...
                            <form action="{{ route('tugas.destroy', $tugas->id) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="inline-flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 rounded-xl text-red-700 font-semibold transition-all duration-300 hover:scale-105">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="grid grid-cols-2 gap-4 mt-8 p-6 bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl">
                    ...
                    <div>
                        ...
                        @if (!($isGuru ?? false) && $submission)
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-lg bg-{{ $submission->statusColor }}-100 text-{{ $submission->statusColor }}-700 text-sm font-semibold">
                                {{ $submission->statusText }}
                            </span>
                        @elseif(!($isGuru ?? false))
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-lg bg-yellow-100 text-yellow-700 text-sm font-semibold">
                                Belum Dikumpulkan
                            </span>
                        @else
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between gap-4 mb-2">
                                ...
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-{{ $submission->statusColor }}-100 text-{{ $submission->statusColor }}-700">
                                    {{ $submission->statusText }}
                                </span>
                            </div>
                            <p class="text-sm text-green-700 mb-3">Dikumpulkan pada <span
                                    class="font-semibold">@formatDateTime($submission->submitted_at)</span></p>
                                ...
                                <div class="bg-white rounded-xl p-4 mb-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="font-semibold text-gray-900 truncate">{{ $submission->file_name }}
                                            </p>
            ...
            @if (!$isGuru && !$submission)
                ...
                <form action="{{ route('tugas.submit', $tugas->id) }}" method="post" enctype="multipart/form-data"
                    class="space-y-6" x-data="{
                        ...
                        contentText: '',
                        hasInput() { return (this.fileName && this.fileName.length > 0) || (this.contentText && this.contentText.length > 0); }
                    }">
                    @csrf
                    ...
                    <div>
                        ...
                        <!-- Drop Zone -->
                        <div class="relative" @dragenter="isDragging = true" @dragleave="isDragging = false"
                            ...
                            <label for="file-input" class="block cursor-pointer transition-all duration-300"
                                ...
                                <div class="border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-300"
                                    :class="isDragging ? 'border-blue-500 bg-blue-50' : fileName ?
                                    ...
                                    <template x-if="!fileName">
                                        <div class="space-y-3">
                                            <div
                                                class="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500 mt-1">PDF, DOC, DOCX, XLS, PPT, ZIP, atau
                                                    Gambar (max 10MB)</p>
                                            </div>
                                        </div>
                                    </template>
                                    ...
                                    <template x-if="fileName">
                                        <div class="flex items-center gap-4 bg-white rounded-xl p-4">
                                            <div
                                                class="flex-shrink-0 w-12 h-12 rounded-xl bg-green-100 flex items-center justify-center">
                        ...
                        @error('file')
                            <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><svg class="w-4 h-4"
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-3">Catatan / Keterangan
                            (opsional)</label>
                        <textarea name="content" rows="4" @input="contentText = $event.target.value.trim()"
                            class="w-full border border-gray-200 rounded-xl p-4 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200"
                            placeholder="Tambahkan catatan atau keterangan tentang tugas Anda...">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><svg class="w-4 h-4"
                                    fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                    ...
                    <div class="flex items-center gap-4 pt-4">
                        <button type="submit" :disabled="!hasInput()"
                            :class="!hasInput() ? 'opacity-50 cursor-not-allowed' : ''"
                            class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-blue-700 transition-all duration-300 shadow-sm hover:shadow-md">
                        ...
                        <a href="{{ route('tugas') }}"
                            class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors duration-200 font-medium">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1">
                                    <div class="flex items-start justify-between gap-3 mb-2">
                                        ...
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-{{ $sub->statusColor }}-100 text-{{ $sub->statusColor }}-700">
                                            {{ $sub->statusText }}
                                        </span>
                                    </div>
                                    <p class="text-sm text-gray-600 mb-3">Dikumpulkan: @formatDateTime($sub->submitted_at)</p>
                                    ...
                                    @if ($sub->grade)
                                        <div
                                            class="inline-flex items-center gap-2 px-3 py-1.5 bg-green-100 text-green-700 rounded-lg">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
