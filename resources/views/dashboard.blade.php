...
@section('content')
    <div class="space-y-8">
        ...
        <div class="animate-fade-in">
            <div class="bg-linear-to-br from-blue-500 to-blue-600 rounded-2xl shadow-lg overflow-hidden relative">
                <!-- Decorative elements -->
                ...
                <div class="p-8 text-white relative z-10">
                    <div class="pl-8 lg:pl-0">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <span
                                class="px-3 py-1 bg-blue-700/50 backdrop-blur-sm rounded-full text-xs font-semibold">Student
                                Mode</span>
                        </div>
            ...
            @forelse($stats ?? [] as $index => $stat)
                ...
                <div class="group bg-white/80 backdrop-blur-sm p-6 rounded-2xl border border-gray-100 shadow-minimal hover:shadow-minimal-hover transition-all duration-500 ease-out hover:-translate-y-2 animate-fade-in-scale"
                    ...
                    <div class="flex items-start justify-between">
                        ...
                        <div
                            class="p-3 bg-gradient-to-br from-{{ $color }}-50 to-{{ $color }}-100 rounded-xl group-hover:scale-110 transition-transform duration-300">
        ...
        <div class="animate-slide-in-right" style="animation-delay: 400ms;">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('kelas') }}"
                    class="text-blue-600 hover:text-blue-700 font-medium text-sm flex items-center gap-2 transition-colors duration-200">
                    Lihat semua
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($classes ?? [] as $index => $class)
                    @php
                        ...
                        // Palette for named colors (consistent across app)
                        $colorMap = [
                            'blue' => [
                                'from' => '#3b82f6',
                                'to' => '#1e40af',
                        ...
                        // Support custom HEX/RGB colors like "#f59e0b"
                        $namedKeys = array_keys($colorMap);
                        $isCustom = !in_array(strtolower($cColor), $namedKeys);

                        if ($isCustom) {
                            // Use the same color for from/to; use neutral light shades for backgrounds/hover
                            $gradientColors = [
                                'from' => $cColor,
                                'to' => $cColor,
                            ...
                            $gradientColors = $colorMap[strtolower($cColor)] ?? $colorMap['blue'];
                        }
                    @endphp
                            ...
                            <div class="mb-4">
                                <div class="flex justify-between text-xs text-gray-600 mb-2">
                                    ...
                                    <span class="font-bold"
                                        style="color: {{ $gradientColors['to'] }};">{{ $cProgress }}%</span>
                                </div>
                                <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                    <div class="h-2 rounded-full transition-all duration-1500 ease-out shadow-sm"
                    ...
                    <div class="col-span-1 md:col-span-2 lg:col-span-3">
                        <div class="bg-white/80 backdrop-blur-sm p-12 rounded-2xl border border-gray-100 text-center">
                            ...
                            <a href="{{ route('kelas.join') }}"
                                class="inline-block mt-4 px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors duration-200 font-medium">
                                Tambah Kelas
                            </a>
                        </div>
                    </div>
