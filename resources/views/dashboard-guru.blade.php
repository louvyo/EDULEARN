...
@section('content')
    <div class="space-y-8">
        ...
        <div class="animate-fade-in">
            <div class="bg-linear-to-br from-purple-500 to-purple-600 rounded-2xl shadow-lg overflow-hidden relative">
                <!-- Decorative elements -->
                ...
                <div class="p-8 text-white relative z-10">
                    <div class="pl-8 lg:pl-0">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <span
                                class="px-3 py-1 bg-purple-700/50 backdrop-blur-sm rounded-full text-xs font-semibold">Teacher
                                Mode</span>
                        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse($stats ?? [] as $index => $stat)
                ...
                <div class="group bg-white/80 backdrop-blur-sm p-6 rounded-2xl border border-gray-100 shadow-minimal hover:shadow-minimal-hover transition-all duration-500 ease-out hover:-translate-y-2 animate-fade-in-scale"
                    ...
                    <div class="flex items-start justify-between">
                        ...
                        <div
                            class="p-3 bg-linear-to-br from-{{ $color }}-50 to-{{ $color }}-100 rounded-xl group-hover:scale-110 transition-transform duration-300">
                    ...
                    @if ($trend)
                        <div class="mt-4 flex items-center text-sm">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-lg bg-{{ $color }}-50 text-{{ $color }}-700 font-medium">
                                {{ $trend }}
                            </span>
                        </div>
                    @endif
            ...
            @empty
                <div class="col-span-1 sm:col-span-2 lg:col-span-4">
                    <div
                        class="bg-white/80 backdrop-blur-sm p-8 rounded-2xl border border-gray-100 text-center text-gray-500">
        ...
        <div class="animate-slide-in-up" style="animation-delay: 400ms;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <a href="{{ route('kelas.create') }}"
                    class="group bg-linear-to-br from-blue-500 to-blue-600 p-6 rounded-2xl text-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                ...
                <a href="{{ route('tugas.create') }}"
                    class="group bg-linear-to-br from-green-500 to-green-600 p-6 rounded-2xl text-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center justify-between mb-4">
                        <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('nilai') }}"
                    class="group bg-linear-to-br from-orange-500 to-orange-600 p-6 rounded-2xl text-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center justify-between mb-4">
                        <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="animate-slide-in-right" style="animation-delay: 600ms;">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('kelas') }}"
                    class="text-purple-600 hover:text-purple-700 font-medium text-sm flex items-center gap-2 transition-colors duration-200">
                    Lihat semua
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($classes ?? [] as $index => $class)
                    @php
                        ...
                        $isCustomColor = !in_array($cColor, $presetColors);

                        // Define color values for gradient (Tailwind-like palette)
                        $colorMap = [
                            'slate' => [
                                'from' => '#94a3b8',
                                'to' => '#475569',
                        ...
                        $mainColor = $isCustomColor ? $cColor : $gradientColors['to'];
                        $lighterColor = $isCustomColor ? 'rgba(0,0,0,0.05)' : $gradientColors['lighter'];
                    @endphp
                    <div class="group bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-minimal hover:shadow-minimal-hover overflow-hidden transition-all duration-500 ease-out hover:-translate-y-2 animate-fade-in-scale"
                        ...
                        @endif
                        <div class="p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-1 transition-colors duration-300">
                                {{ $cTitle }}</h3>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div class="bg-gray-50 rounded-xl p-3">
                                    ...
                                    <p class="text-xl font-bold" style="color: {{ $mainColor }};">
                                        {{ $totalSiswa }}</p>
                                </div>
                                <div class="bg-gray-50 rounded-xl p-3">
                                    <div class="flex items-center gap-2 mb-1">
                            ...
                            <a href="{{ $cId ? route('kelas.detail', ['id' => $cId]) : '#' }}"
                                ...
                                style="background-color: white; color: {{ $mainColor }};"
                                onmouseover="this.style.backgroundColor='{{ $lighterColor }}'"
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 lg:col-span-3">
                        <div class="bg-white/80 backdrop-blur-sm p-12 rounded-2xl border border-gray-100 text-center">
                            ...
                            <button
                                class="mt-4 px-6 py-2.5 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-colors duration-200 font-medium">
                                Buat Kelas Pertama
                            </button>
                        </div>
                    </div>
        ...
        @if (isset($tugasTerbaru) && $tugasTerbaru->count() > 0)
            <div class="animate-slide-in-left" style="animation-delay: 800ms;">
                ...
                <div
                    class="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-minimal overflow-hidden">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-gray-600 mb-2">
                                            {{ $tugas->kelas->nama ?? 'Kelas tidak diketahui' }}</p>
                                    ...
                                    <a href="{{ route('tugas.show', $tugas->id) }}"
                                        class="px-4 py-2 bg-purple-50 text-purple-600 rounded-xl hover:bg-purple-100 transition-colors duration-200 font-medium text-sm">
                                        Lihat Detail
                                    </a>
                                </div>
                            </div>
